...
# Methodology dialog (always accessible from sidebar)
st.sidebar.markdown("---")

@st.dialog("CTA Health Scoring Methodology", width="large")
...
if st.sidebar.button("📖 Scoring Methodology", use_container_width=True):
    show_methodology()
...
with tab3:
    if is_competitor and competitor_data:
        ...
        if selected_brand == "ServiceNow":
            st.warning(
                "**ServiceNow data is inconclusive.** Their blog (servicenow.com/workflow/) uses Akamai WAF "
                "and Adobe Experience Manager with custom web components (shadow DOM). This blocks all "
                "programmatic crawling — curl, Python, headless browsers, and most AI search engines all "
                "receive 403 errors. Article count is verified via an undocumented AEM content API, but "
                "section-level CTA analysis is not possible without full browser rendering of each page. "
                "Scores shown are estimates based on limited extraction."
            )
